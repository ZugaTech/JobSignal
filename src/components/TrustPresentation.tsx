    ...
    <div className="space-y-4 pt-2 border-t border-border/40">
      ...
      {report.total_signal_count > 0 ? (
        <p className="text-xs text-neutral-500 leading-snug">
          Structured checks resolved: {report.verified_signal_count}/{report.total_signal_count} — evidence completeness{' '}
          <span className="tabular-nums text-neutral-300">{report.evidence_completeness_score}/100</span> matches that
          same ratio (recomputed on every response, including cache hits, from the signals in this payload).
        </p>
      ) : (
        <p className="text-xs text-neutral-500 leading-snug">
          Evidence completeness:{' '}
          <span className="tabular-nums text-neutral-300">{report.evidence_completeness_score}/100</span> (no check
          denominator for this run). Separate from overall strength above.
        </p>
      )}
