...
ENV_SPECS: tuple[EnvVarSpec, ...] = (
    ...
    EnvVarSpec(
        ...
        _as_int,
        16_000,
        "Max characters of cleaned main-body text appended to extracted_job_text after meta/JSON-LD hints",
    ),
    ...
    EnvVarSpec("RATE_LIMIT_REQUESTS_PER_MINUTE", _as_int, 18, "Per-IP request quota per minute"),
    EnvVarSpec("RATE_LIMIT_BURST", _as_int, 5, "Per-IP additional burst allowance within a window"),
    EnvVarSpec("SEARCH_API_ENDPOINT", str, "https://google.serper.dev/search", "Serper provider endpoint URL"),
    EnvVarSpec("SEARCH_TIMEOUT_S", _as_int, 5, "Serper request timeout in seconds"),
    EnvVarSpec("SEARCH_RETRY_COUNT", _as_int, 1, "Serper retry attempts"),
    EnvVarSpec("SEARCH_RATE_LIMIT_PER_MINUTE", _as_int, 60, "Serper provider request cap per minute"),
    EnvVarSpec(
        "SEARCH_MAX_CALLS_EVIDENCE",
        _as_int,
        8,
        "Max Serper POST calls per verify for careers/board/registry/duplicate queries (pipeline runs 6 parallel searches)",
    ),
    EnvVarSpec(
        "SEARCH_MAX_CALLS_REPUTATION",
        _as_int,
        8,
        "Max Serper POST calls per verify for company reputation (6 parallel plain queries; keep ≥6)",
    ),
    EnvVarSpec(
        "SEARCH_MAX_CALLS_RECOMMENDATIONS",
        _as_int,
        8,
        "Max Serper POST calls when similar-job recommendations are requested",
    ),
    EnvVarSpec("SERPER_API_KEY", str, None, "Primary Serper.dev provider API key"),
    EnvVarSpec("SERPAPI_API_KEY", str, None, "SerpApi Google JSON API key; used when Serper fails or Serper key is unset"),
    EnvVarSpec(
        "SERPAPI_SEARCH_ENDPOINT",
        str,
        "https://serpapi.com/search.json",
        "SerpApi search endpoint (GET + api_key query param)",
    ),
    EnvVarSpec("SEARCH_API_KEY", str, None, "Alias for SERPER_API_KEY only (not used for SerpApi)"),
