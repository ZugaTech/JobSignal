...
REASON_PLAIN_BY_CODE: Dict[str, str] = {
    ...
    "FETCH_RETRY_BROWSER_UA": "The site first blocked our default fetch; we retried once with a generic browser profile.",
    "FETCH_TIMEOUT": "The posting page took too long to respond, so we moved on with partial context.",
    "FETCH_ERROR": "We could not complete a network request to the posting page.",
    "FETCH_BLOCKED": "We could not start a fetch to that URL (policy or safety checks).",
    "FETCH_HTTP": "The posting URL returned an error status instead of a normal page.",
    "FETCH_CONTENT_TYPE": "The posting URL did not return HTML we could parse as a job page.",
    "FETCH_EMPTY_EXTRACT": "The page responded, but we could not extract readable job text (often anti-bot or minimal HTML).",
    "FETCH_DOMAIN_SHIFT": "The posting redirected to a different site than the original URL suggested.",
    "FETCH_REDIRECT_LIMIT": "The posting redirected too many times, so we stopped following.",
    "FETCH_REDIRECT": "The posting returned a redirect we could not follow.",
    "FETCH_REDIRECT_SSRF": "A redirect pointed somewhere we are not allowed to fetch.",
