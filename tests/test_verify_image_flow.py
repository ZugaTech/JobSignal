...
@pytest.mark.asyncio
async def test_verify_job_image_only_blocked_when_vision_empty(monkeypatch):
    monkeypatch.setattr(
        "backend.core.image_ingest.extract_job_fields_from_image_vision",
        lambda **kwargs: (None, [{"code": "VISION_DISABLED", "message": "disabled"}]),
    )
    r = await orchestrator.verify_job(None, None, image_bytes=MINI_PNG, image_media_type="image/png")
    assert r["verdict"] == "VERIFY"
    assert r["ingestion"]["status"] == "insufficient"

@pytest.mark.asyncio
async def test_verify_job_image_only_proceeds_with_stubbed_vision(monkeypatch):
